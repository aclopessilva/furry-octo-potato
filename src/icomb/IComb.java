 ...
 * @version Version 0.8.9 in 08/08/20 13- all files in a single JAR (iComb.jar includes all universes)
...
/**
 * Esta é classe principal do programa e está implementada na forma
 * <b>superapplet</b>, ou seja, funciona tanto como applet quanto como uma
 * aplicação independente. Instancia todos os participantes e estabelece seus
 * relacionamentos.
 */
public class IComb extends Applet {
    ...
    public static final String UNIVERSE1 = "baralho/baralho.xml",
            UNIVERSE2 = "futebol/futebol.xml";

    public static IComb getInstance() {
        return instanceIcomb;
    }
    private static IComb instanceIcomb = null;
    private static boolean notSend = false;

    public static boolean notSend() {
        return notSend;
    }
    ...
    private void getContentFromStr(String strContent) {
        ...
        if (solucao == null || solucao == "") {
            solucao = (String) hash.get("solucao");
        }
    ...
    private void iniciaParam() {
        try {
            ...
            if (MA_PARAM_notSEND == null || MA_PARAM_notSEND.trim().equals("")) {
                notSend = false;
            } else if (MA_PARAM_notSEND.equalsIgnoreCase("true")) {
                notSend = true;
            } else {
                notSend = false;
            }
            ...
            if (MA_proposition != null && MA_proposition.length() > 2) {
                ...
                if (MA_proposition == null || MA_proposition == "") {
                    ...
                    if (solucao == null || solucao == "") {
                        solucao = getParameter("solucao");
                    }
                    resposta = getParameter("answer_value");
                    if (resposta == null || resposta == "") {
                        resposta = getParameter("resposta");
                    }
    ...
    private void monta() {
        ...
        centralPanel = new Panel(new BorderLayout()) { // PANEL_WIDTH = 650
            public Dimension getPreferredSize() {
                return new Dimension(Constants.PANEL_WIDTH, Configuracoes.ALT);
            }
        };
...
//??    if (solucao!=null && resposta!=null) { // when iComb is started with no exercise => 'solucao' and 'resposta' are 'null' => must avoid lines bellow
            ...
            System.out.println("icomb/IComb.monta(): NAO carregou ObjectManager.setMachine.loadUniverso ------------");
        }
